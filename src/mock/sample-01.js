...
const SearchContainer = ({ t }) => (
  <div className={"soco-search-container"} style={{}}>
    <AutoComplete
...
function getInit(t, tab, ifQuery) {
  fetch(socoUrl + "/api/tasks/soco/init/" + process.env.QUERY_API_KEY, {
    ...
    .then(function (json) {
      ...
      if (json.hasOwnProperty("sentiments")) sentiments = json.sentiments;
      ...
      if (!ifQuery) {
        ...
        if (sentiments.category) {
          ...
          meta_distribution = meta_distribution.filter(
            (m) => !m.type.includes("_id") && m.type[0] !== "_"
          );
        ...
        if (tab === "kt")
          setTimeout(
            KnowledgeTree(json.tree, window.innerWidth * 0.7, "#report-knowledge-tree"),
            1000
          );
      }
  ...
  fetch(socoUrl + "/api/tasks/soco/find", {
    ...
    .then(function (json) {
      if (json[0]) {
        ...
        // Configs
        let configs = json[0].configs.filter(
          (c) => c.key === "graph" || c.key === "tree" || c.key === "meta_dist"
        );
        if (configs.length !== 0) {
          configs.forEach((c) => {
...
class AggregateSearch extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      dataSource: example.promoted_data,
      n_best: 120,
      ifEmbed: false,
      use_embed: true,
      keep_vectors: true,
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    // Cluster Tab
    if (
      !this.state.current_cluster &&
      this.state.current_cluster !== prevState.current_cluster &&
      this.state.prev_cluster
    ) {
      // Scroll
  ...
  handleCheckedFilters = (type, label, checked) => {
    ...
    if (!filters_content.hasOwnProperty(type)) filters_content[type] = [];
    if (label === "No " + type) label = "";
    if (checked)
      if (!filters_content[type].includes(label)) filters_content[type].push(label);
      else {
      }
    else filters_content[type] = filters_content[type].filter((l) => l !== label);
    if (filters_content[type].length === 0) delete filters_content[type];
    this.setState({ filters_content });
  };
  // Autocompletion
      ...
      fetch(socoUrl + "/api/soco-aggregate/" + process.env.QUERY_API_KEY, {
        ...
        .then(function (json) {
          if (!json.hasOwnProperty("error")) {
            ...
            // Cluster
            let cluster = json.agg_results.hasOwnProperty("cluster")
              ? json.agg_results.cluster
              : {};
            ...
            if (answers.length !== 0) {
              ...
              if (window.innerWidth >= 768) self.setState({ tab: "kt" });
              else self.setState({ tab: "kw" });
            }
            ...
            if (self.state.tab === "kt")
              setTimeout(
                KnowledgeTree(tree, window.innerWidth * 0.7, "#report-knowledge-tree"),
                1000
              );
          } else {
  ...
  handleQuickSearch = (value, source, filters_content) => {
    ...
    if (source) self.handleSearchChange(value);
  ...
  handleLinkAndNode = (data) => {
    ...
    links.sort(function (a, b) {
      if (a.source.weight > b.source.weight) {
        return 1;
      } else if (a.source.weight < b.source.weight) {
        return -1;
      } else {
        if (a.target.weight > b.target.weight) {
          return 1;
        }
        if (a.target.weight < b.target.weight) {
          return -1;
    ...
    links.forEach(function (link) {
      link.source =
        ...
        (nodes[link.source] = {
          name: link.source,
          root: link.source === data.root || link.type === "co-occur",
        });
    ...
    return (
      ...
      >
        <div className={"soco-report-container"} style={{ width: "100%", overflow: "hidden" }}>
          ...
          <div className={"soco-report-banner soco-aggregate-search-banner"} style={{}}>
            {window.innerWidth >= 768 ? (
              ...
              >
                <div
                  className={"soco-list"}
                  style={{ marginLeft: "20px", justifyContent: "flex-start", width: "60%" }}
                >
                  <Logo t={this} />
                  ...
                  >
                    <div className={"soco-list_item"}>
                      <img src="pic/svg/filter.svg" alt={"soco-filter"} />
                    </div>
                    <div
                      className={"soco-list_item"}
                      style={{ marginLeft: "15px", fontSize: "16px", color: "#000" }}
                    >
                      Filter
                    </div>
                  </div>
          ...
          <div className={"soco-report-body soco-aggregate-search"} style={{}}>
            {/* Link for Download */}
            ...
            <Row style={{ width: "100%", height: "100%", padding: "5px 20px" }}>
                ...
                {loading ? (
                  ...
                  <StickyContainer style={{ height: "100%" }}>
                    ...
                    >
                      ...
                      {answers.length !== 0 && (
                        <TabPane
                          className={"soco-tabpane soco-scroll-tabpane"}
                          id={"soco-tabpane-qa"}
                          tab="QA"
                          key="qa"
                        ...
                        >
                          <QA
                            ...
                            a_dislikes={a_dislikes}
                            a_likes={a_likes}
                            handleDownload={this.handleDownload}
                            handleQARelatedArgvs={this.handleQARelatedArgvs}
                            handleQuickSearch={this.handleQuickSearch}
                          />
                        </TabPane>
                      )}
                      {/* Response Cluster */}
                      {Object.entries(cluster).length !== 0 && answers.length !== 0 && (
                        <TabPane
                          className={"soco-tabpane soco-scroll-tabpane"}
                          id={"soco-tabpane-ac"}
                          tab={window.innerWidth >= 768 ? "Response Cluster" : "Cluster"}
                          key="ac"
                          style={{ height: "100%", overflowY: "scroll" }}
                        >
                          <ActionCluster
                            ...
                            onClusterOpen={this.onClusterOpen}
                            onClusterClose={this.onClusterClose}
                            handlePageChange={this.handlePageChange}
                          />
                        </TabPane>
                      )}
                      {/* Meta Distribution */}
                      {showMD && meta_distribution.length !== 0 && (
                        <TabPane
                          className={"soco-tabpane soco-scroll-tabpane"}
                          tab={window.innerWidth >= 768 ? "Meta Distribution" : "Meta"}
                          key="md"
                          style={{ height: "100%", overflowY: "scroll" }}
                        >
                          <ComponentAnalysis
                      ...
                      {window.innerWidth < 768 && keywords.length > 0 && (
                        <TabPane
                          className={"soco-tabpane soco-scroll-tabpane"}
                          tab="Keywords"
                          key="kw"
              ...
              {window.innerWidth >= 768 && (
                <Col
                  ...
                  xl={7}
                  xxl={7}
                  style={{ padding: "50px 20px 0 30px", height: "100%", overflowY: "scroll" }}
                >
